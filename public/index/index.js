    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        socket.on('previous-images', (data) => {
            const fileURLs = data.fileURL;
            const username = data.userName
            console.log(fileURLs)
            
            fileURLs.forEach(async (file) => {
                const filename = file.fileName; 
                const fileurl = file.fileURL; 
                const timestamp = await convertS3TimestampToNormalTime(file.timestamp)
                console.log('previous', file); 
                console.log('usrname', username)
                // Display the uploaded file
                displayUploadedFile(filename, fileurl, username, timestamp);
            });
        })
        async function convertS3TimestampToNormalTime(timestamp) {
        ...
        function displayUploadedFile(fileName, fileURL, username, time) {
            ...
            if (fileName.toLowerCase().endsWith('.png') || fileName.toLowerCase().endsWith('.jpg') || fileName.toLowerCase().endsWith('.jpeg') || fileName.toLowerCase().endsWith('.gif')) {
                ...
                imageElement.className = isSent ? 'sent-image' : 'received-image';
